...
class Node:
    ...
    * children (list of Nodes) - the list of child-nodes, related with next possible moves
    ...
    def __init__(self, parent_board: SudokuBoard, depth: int = 0, move: Move = None, points: float = 0):
        ...
        self.score = None  # evaluation minimax score
    ...
    def add_children(self, taboo_moves: list):
        ...
        if self.children is None:
            ...
            self.children = [Node(parent_board=self.board, depth=self.depth + 1, move=x_move,
                                  points=evaluate(move=x_move, current_board=self.board)) for x_move in legal_moves]

    def add_level(self, taboo_moves: list, target_depth: int):
        """
        Function, that calls for all leaf nodes the function that produce children (expanding the tree to one more
        possible level)
        @param taboo_moves - taboo_moves - the list of taboo moves
        @param target_depth - the target depth of tree expansion
    ...
    def update_score(self):
        """
        A minimax function for the tree, that upwards from the leaves updates the score (difference between points
        agent AI gets and the opponent gets). All nodes located on the even levels (except root) represents the moves
        that gives points to the opponent (negative scores, to maximize as for the agent), all nodes on the odd
        levels represents the moves that gives points to the AI agent (positive scores, to minimize as for the opponent).
