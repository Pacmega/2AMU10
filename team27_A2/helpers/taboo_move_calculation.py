...
def locked_candidates_rows(game_state: GameState,
    ...
    """
    # print(allowed_in_rows)
    # print(allowed_in_blocks)
    ...
    for i in range(0, game_state.board.N, columns_per_block):
        ...
        for number in incomplete_numbers:
            ...
            # Iterate over sudoku blocks within the previous for loop's columns
            #   of blocks from top to bottom (so now we look at specific blocks)
                    ...
                    for row in rows_to_check:
                        for j in range(block[1], block[1] + columns_per_block):
                            if (row, j) in legal_moves and number in legal_moves[(row, j)]:
                                ...
                                taboo_moves[(row, j)].append(number)
...
def locked_candidates_columns(game_state: GameState,
                              ...
                              taboo_moves: defaultdict[Tuple[int, int], List[int]]):
    """
    Taboo move detection heuristic to detect locked candidates
    @param game_state:         The current GameState whose board to check for taboo moves with.
    @param legal_moves:        The dictionary of current legal moves that should be checked for hidden singles.
...
def obvious_singles(game_state: GameState,
    ...
    @param legal_moves: The dictionary of current legal moves that should be checked for hidden singles.
    @param taboo_moves: The dictionary (defaultdict) of currently known taboo moves for this GameState.
    @return:            A new list containing all found taboo moves. The legal_moves list
                        is edited in-place.
    """
